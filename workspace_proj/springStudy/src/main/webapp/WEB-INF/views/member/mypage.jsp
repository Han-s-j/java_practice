                    ...
                    <div class="col-lg-8 col-xl-7">
                        <form id="contactForm" action="${pageContext.request.contextPath}/loginDo" method="post" data-sb-form-api-token="API_TOKEN">
                            ...
                            <div class="form-floating mb-3">
                                <input class="form-control" name="memId" value="${cookie.rememberId.value }" type="text" placeholder="아이디" data-sb-validations="required" />
                                <label for="">아이디</label>
                            </div>
                            <!-- Email address input-->
                            <div class="form-floating mb-3">
                                <input class="form-control" name="memPw" type="password" placeholder="패스워드" data-sb-validations="required,email" />
			...
			$("#uploadImage").on("change", function(){
				...
				if(file){
					...
					if(!checkType.includes(fileType)){
						...
						// 전송
						var formDate = new FormData($("#profileForm")[0]);
						$.ajax({
							url : '<c:url value="/files/upload" />'
							,type: 'POST'
							,data: formDate
							,processData:false  // 전송 객체를 URL인코딩 하지 않도록
							,contentType:false  // 파일을 이전 데이터 형태로 전동하기 위해
